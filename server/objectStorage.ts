...
export class ObjectStorageService {
  ...
  async getProfilePictureFile(avatarPath: string): Promise<File | null> {
    ...
    const fullPath = `${privateDir}${avatarPath.slice(1)}`; // Remove leading slash
    const { bucketName, objectName } = parseObjectPath(fullPath);
    const bucket = objectStorageClient.bucket(bucketName);
    const file = bucket.file(objectName);
  ...
  normalizeProfilePicturePath(rawPath: string, userId: string): string {
