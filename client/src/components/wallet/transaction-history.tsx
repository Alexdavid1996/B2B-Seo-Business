...
export default function TransactionHistory({ transactions }: TransactionHistoryProps) {
  ...
  // Hardcoded transactions per page
  const transactionsPerPage = 5;

  // Fetch wallet transactions with pagination
  const { data: walletResponse } = useQuery({
    queryKey: ["/api/wallet-transactions", currentPage, transactionsPerPage],
    queryFn: async () => {
      const response = await fetch(`/api/wallet-transactions?page=${currentPage}&limit=${transactionsPerPage}`);
  ...
  console.log("Current walletPagination:", walletPagination);
  ...
  const allTransactions = [
    ...
    ...walletTransactions.map((wt: WalletTransactionData) => ({ 
      ...wt, 
      source: 'wallet_transaction',
      description: wt.type === 'top_up' 
        ? `Top-up: ${formatCurrency(wt.amount)} via Cryptocurrency`
        : `Withdrawal: ${formatCurrency(wt.amount)} via Cryptocurrency`,
      type: wt.type
    }))
  ].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  // Use wallet pagination if available, otherwise fall back to client-side
  const totalPages = walletPagination?.totalPages || Math.ceil(allTransactions.length / transactionsPerPage);
  const startIndex = walletPagination ? ((walletPagination.page - 1) * walletPagination.limit) : ((currentPage - 1) * transactionsPerPage);
  const endIndex = walletPagination ? (startIndex + walletPagination.limit) : (startIndex + transactionsPerPage);
  const total = walletPagination?.total || allTransactions.length;
  
  // For display purposes - if we have server pagination, show only wallet transactions
  // Otherwise show paginated combined results
  const displayTransactions = walletPagination ? 
    walletTransactions.map((wt: WalletTransactionData) => ({ 
      ...wt, 
      source: 'wallet_transaction',
      description: wt.type === 'top_up' 
            ...
            {walletPagination && (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-6 pt-4 border-t gap-4 sm:gap-0">
                <div className="text-xs sm:text-sm text-gray-600 text-center sm:text-left">
                  Showing {Math.min(walletPagination.limit * (walletPagination.page - 1) + 1, parseInt(walletPagination.total))}-{Math.min(walletPagination.page * walletPagination.limit, parseInt(walletPagination.total))} of {walletPagination.total} transactions
                  <span className="text-gray-400 ml-2 hidden sm:inline">({walletPagination.limit} per page)</span>
                </div>
                {walletPagination.totalPages > 1 && (
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handlePageChange(walletPagination.page - 1)}
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: walletPagination.totalPages }, (_, i) => i + 1).map((page) => (
                        <Button
                          key={page}
                          variant={walletPagination.page === page ? "default" : "outline"}
                          size="sm"
